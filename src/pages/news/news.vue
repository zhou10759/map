<template>
  <div>
    ...
    <!-- 新闻列表 -->
    <div class="news-list" v-for="(news,index) in newsList.news" :key="index" @click="toDetail(news)">
      <div class="news-title">{{news.newsTitle}}</div>
      <div class="news-content">
        <img :src="news.newsImg" alt class="news-img" />
        <div class="news-cont" v-html="newsCont[index+((currentPage-1)*10)]">{{newsCont[index]}}</div>
      </div>
    </div>
    <!-- 分页器 -->
...
<script>
  ...
  import { initNews } from "@/api/index/index.js";
...
<style scoped>
  ...
  .news-push {
    ...
    height: 600px;
    line-height: 600px;
    background-image: url("../../../static/img/Bitmap@2x(1).png");
